  ...
  return new TextPrompt({
    ...
    render() {
      ...
      switch (this.state) {
        ...
        case "submit":
          return `${title} ${color.dim(value || opts.placeholder)}`;
        case "cancel":
...
export const group = async <T>(
  ...
  const results = {} as any; //eslint-disable-line
  ...
  for (const name of promptNames) {
    ...
    if (typeof opts?.onCancel === "function" && isCancel(result)) {
      results[name] = "canceled"; //eslint-disable-line
      opts.onCancel({ results }); //eslint-disable-line
      continue;
    }

    results[name] = result; //eslint-disable-line
  }

  return results; //eslint-disable-line
};
